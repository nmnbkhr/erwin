...
import { Database, Radio, Server, Brain, Monitor, Shield, ChevronDown, ChevronUp, Cpu, Code, LayoutDashboard, Workflow, GitBranch } from 'lucide-react'
import type { WorkbenchData } from '../../utils/profitabilityWorkbench'

interface Props {
  data: WorkbenchData['technologyArchitecture']
}
...
const TECH_ICONS: Record<string, typeof Cpu> = {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <h2 className="text-lg font-semibold text-slate-800 mb-4">4-Layer System Architecture</h2>
        <div className="space-y-3">
          {data.layers.slice().reverse().map((layer, revIdx) => {
            const idx = data.layers.length - 1 - revIdx
            const config = LAYER_CONFIG[idx]
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {['Source Systems', 'CDC / Streaming', 'Data Lake', 'EDW / Star Schema', 'Analytics Engine', 'Consumption'].map((step, i) => {
            ...
            return (
              <div key={step} className="flex items-center gap-2">
                ...
                {i < 5 && <div className="text-blue-400 text-xl font-bold shrink-0">&rarr;</div>}
              </div>
            )
          })}
        </div>
